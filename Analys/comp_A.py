...
if __name__ == "__main__":
    ...
    print(df_before['date'].tolist())

    df_before['date'] = df_before['date'].map(lambda x: str(int(x)))
    #
    # merge MSM & BEFORE data
    #

    df = pd.merge(df_msm, df_before, on = 'date', how = 'inner')
    print(df['CLING'])
    ...
    plt.plot(df.index, df['CIG'], label='MSM(eq.)')
    plt.plot(df.index, df['CLING'], label='BEFORE')
